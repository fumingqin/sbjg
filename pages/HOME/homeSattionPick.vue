			...
			<view class="main" v-if="isShowAllList">
				<swiper class="swiper" :style="{ 'height':scrollHeight }" 
					...
					<swiper-item v-for="(item,index) in stationArray" :key="index">
						<scroll-view  scroll-y="true" :style="{ 'height':scrollHeight }">
							<view class="item">
								<view class="goods" v-for="(item2,index2) in stationArray[index].locations" :key="index2" @tap="detailStationTap(item2)">
									<view>
										<view>{{item2.Memo}}</view>
									</view>
								</view>
							</view>
						</scroll-view>
	...
	export default {
		...
		methods: {
			...
			getBusStationList() {
				uni.showLoading();
				uni.request({
					url: $Sbjg.SbjgInterface.GetCompany.Url,
					method: $Sbjg.SbjgInterface.GetCompany.method,
					header: $Sbjg.SbjgInterface.GetCompany.header,
					success: (res) => {
						uni.hideLoading();
						let that = this;
						// console.log(res.data);
						if (res.data.length != 0) {
								this.stationArray=res.data;
						}
			...
			drawCorrelativeKeyword(keywords, keyword) {
				...
				var len = keywords.length,
					keywordArr = [];
				for (var i = 0; i < len; i++) {
					var row = keywords[i].companyname;
					//定义高亮#9f9f9f
